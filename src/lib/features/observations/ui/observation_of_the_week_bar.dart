      ...
      child: BlocBuilder<ObservationOfTheWeekBarCubit, ObservationOfTheWeekBarState>(
        builder: (context, state) {
          if (state is ObservationOfTheWeekBarInitial) {
            return Container();
          } else if (state is ObservationOfTheWeekBarLoaded) {
            return InkWell(
              onTap: () {
                GetIt.I<AnalyticsService>().logEvent(name: Keys.analyticsShowObservationOfTheWeek);
                Navigator.of(context).push(MaterialPageRoute(
                  builder: (context) => const AnswerObservationScreen(),));
              },
              child: Material(
                elevation: 1,
                child: Container(
                 ...
                 */
                        Expanded(
                          child: Column(children: [
                            const Text("Frage der Woche:"),
                            Text('"${(state).question.text.replaceAll('"', "'")}"', textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.bold),),
                          ],),
                        ),
                        /*
